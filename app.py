...
def run_market_basket_analysis(df, target_month, min_support, min_confidence):
    """
    تقوم هذه الدالة بعمل تحليل سلة المشتريات الكامل لشهر معين.
    """
    monthly_df = df[df['YearMonth'] == target_month]
    
    if monthly_df.empty:
        return f"لا توجد بيانات مبيعات للشهر المحدد: {target_month}", "error"
    ...
    # تنظيف وتنسيق جدول النتائج
    rules['antecedents'] = rules['antecedents'].apply(lambda x: ', '.join(list(x)))
    rules['consequents'] = rules['consequents'].apply(lambda x: ', '.join(list(x)))
    
    # اختيار وترتيب الأعمدة المهمة
...
if uploaded_file is not None:
    ...
    if error_msg: st.error(error_msg)
        ...
        if st.button("🔍 اكتشف علاقات المنتجات", type="primary"):
            ...
            elif status == "error": st.error(result)
